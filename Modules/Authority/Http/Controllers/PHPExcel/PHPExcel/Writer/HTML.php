                ...
                if (!isset($this->isSpannedRow[$sheet->getParent()->getIndex($sheet)][$row])) {
                    ...
                    while ($column++ < $dimension[1][0]) {
                        ...
                        if ($sheet->cellExistsByColumnAndRow($column, $row)) {
                            $rowData[$column] = HTML . phpPHPExcel_Cell::stringFromColumnIndex($column);
                        } else {
                            $rowData[$column] = '';
                        }
                    }
        ...
        foreach ($pSheet->getChartCollection() as $chart) {
            if ($chart instanceof PHPExcel_Chart) {
                ...
                if ($chartCoordinates['cell'] == $coordinates) {
                    $chartFileName = PHPExcel_Shared_File::sys_get_temp_dir() . 'HTML.php/' .uniqid().'.png';
    ...
    private function generateRow(PHPExcel_Worksheet $pSheet, $pValues = null, $pRow = 0, $cellType = 'td')
    {
        if (is_array($pValues)) {
            ...
            foreach ($pValues as $cellAddress) {
                ...
                $coordinate = HTML . phpPHPExcel_Cell::stringFromColumnIndex($colNum);
                if (!$this->useInlineCss) {
                    $cssClass = '';
                ...
                if (isset($this->isBaseCell[$pSheet->getParent()->getIndex($pSheet)][$pRow + 1][$colNum])) {
                    ...
                    $endCellCoord = HTML . phpPHPExcel_Cell::stringFromColumnIndex($colNum + $colSpan - 1);
                    if (!$this->useInlineCss) {
