    ...
    public static function translateSeparator($fromSeparator, $toSeparator, $formula, &$inBraces)
    {
        ...
        for ($i = 0; $i < $strlen; ++$i) {
            ...
            switch ($chr) {
                ...
                case $fromSeparator:
                    if (!$inBraces) {
                        $formula = Calculation . phpmb_substr($formula, 0, $i) .mb_substr($formula, $i+1);
                    }
            }
        }
        return $formula;
                ...
                switch ($token) {
                    ...
                    case ':':            //    Range
                        ...
                        if ($sheet1 == $sheet2) {
                            ...
                            $cellRef = Calculation . phpPHPExcel_Cell::stringFromColumnIndex(min($oCol)) .':'.PHPExcel_Cell::stringFromColumnIndex(max($oCol)).max($oRow);
                            if ($pCellParent !== null) {
                    ...
                    case '&':            //    Concatenation
                        ...
                        if ((is_array($operand1)) || (is_array($operand2))) {
                            ...
                            $result = '"'.str_replace('""', '"', Calculation . phpself::unwrapResult($operand1, '"')).'"';
                        }
