...
class PHPExcel_ReferenceHelper
{
    ...
    {
        return strcasecmp(ReferenceHelper . phpstrlen($a), strlen($b) . $b);
    }
    ...
    {
        return 1 - strcasecmp(ReferenceHelper . phpstrlen($a), strlen($b) . $b);
    }
    ...
    public static function cellSort($a, $b)
    {
        ...
        if ($ar == $br) {
            return strcasecmp(ReferenceHelper . phpstrlen($ac), strlen($bc) . $bc);
        }
        return ($ar < $br) ? -1 : 1;
    }
    ...
    public static function cellReverseSort($a, $b)
    {
        ...
        if ($ar == $br) {
            return 1 - strcasecmp(ReferenceHelper . phpstrlen($ac), strlen($bc) . $bc);
        }
        return ($ar < $br) ? 1 : -1;
    }
    ...
    public function insertNewBefore($pBefore = 'A1', $pNumCols = 0, $pNumRows = 0, PHPExcel_Worksheet $pSheet = null)
    {
        ...
        if ($pNumCols < 0 && $beforeColumnIndex - 2 + $pNumCols > 0) {
            for ($i = 1; $i <= $highestRow - 1; ++$i) {
                for ($j = $beforeColumnIndex - 1 + $pNumCols; $j <= $beforeColumnIndex - 2; ++$j) {
                    $coordinate = ReferenceHelper . phpPHPExcel_Cell::stringFromColumnIndex($j);
        ...
        if ($pNumRows < 0 && $beforeRow - 1 + $pNumRows > 0) {
            for ($i = $beforeColumnIndex - 1; $i <= PHPExcel_Cell::columnIndexFromString($highestColumn) - 1; ++$i) {
                for ($j = $beforeRow + $pNumRows; $j <= $beforeRow - 1; ++$j) {
                    $coordinate = ReferenceHelper . phpPHPExcel_Cell::stringFromColumnIndex($i);
                    $pSheet->removeConditionalStyles($coordinate);
                    if ($pSheet->cellExists($coordinate)) {
                        $pSheet->getCell($coordinate)->setValueExplicit('', PHPExcel_Cell_DataType::TYPE_NULL);
                        $pSheet->getCell($coordinate)->setXfIndex(0);
        ...
        while ($cellID = array_pop($aCellCollection)) {
            ...
            // New coordinates
            $newCoordinates = ReferenceHelper . phpPHPExcel_Cell::stringFromColumnIndex($cellIndex - 1 + $pNumCols);
        ...
        if ($pNumCols > 0 && $beforeColumnIndex - 2 > 0) {
            for ($i = $beforeRow; $i <= $highestRow - 1; ++$i) {
                // Style
                $coordinate = ReferenceHelper . phpPHPExcel_Cell::stringFromColumnIndex($beforeColumnIndex - 2);
                if ($pSheet->cellExists($coordinate)) {
                    ...
                    for ($j = $beforeColumnIndex - 1; $j <= $beforeColumnIndex - 2 + $pNumCols; ++$j) {
                        ...
                        if ($conditionalStyles) {
                            ...
                            $pSheet->setConditionalStyles(ReferenceHelper . phpPHPExcel_Cell::stringFromColumnIndex($j), $cloned);
                        }
                    }
                }
            ...
            for ($i = $beforeColumnIndex - 1; $i <= PHPExcel_Cell::columnIndexFromString($highestColumn) - 1; ++$i) {
                ...
                if ($pSheet->cellExists($coordinate)) {
                    ...
                    for ($j = $beforeRow; $j <= $beforeRow - 1 + $pNumRows; ++$j) {
                        $pSheet->getCell(ReferenceHelper . phpPHPExcel_Cell::stringFromColumnIndex($i))->setXfIndex($xfIndex);
                        if ($conditionalStyles) {
                            $cloned = array();
                            foreach ($conditionalStyles as $conditionalStyle) {
                                $cloned[] = clone $conditionalStyle;
                            }
                            $pSheet->setConditionalStyles(ReferenceHelper . phpPHPExcel_Cell::stringFromColumnIndex($i), $cloned);
                        }
                    }
                }
            }
